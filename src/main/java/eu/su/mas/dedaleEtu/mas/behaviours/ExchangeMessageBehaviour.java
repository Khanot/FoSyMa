...
public class ExchangeMessageBehaviour extends TickerBehaviour {
    ...
    protected void onTick() {
        ...
        ACLMessage mapRequest = myAgent.receive(TEMPLATE_MAP_REQUEST);
        if (mapRequest != null) {
            String requester = mapRequest.getSender().getLocalName();
            System.out.println("[" + myAgent.getLocalName() + "] Demande de carte de " + requester);
            ...
            try {
                ...
                ((AbstractDedaleAgent) myAgent).sendMessage(response);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
