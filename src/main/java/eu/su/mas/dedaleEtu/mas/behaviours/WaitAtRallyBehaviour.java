...
public class WaitAtRallyBehaviour extends SimpleBehaviour {
    ...
    public void action() {
        ...
        if (!assignmentDone && arrivedAgents.containsAll(allAgentNames)) {
            Map<String, Integer> assignment = computeGroups(new ArrayList<>(allAgentNames));
    ...
    private Map<String, Integer> computeGroups(List<String> agents) {
        int nbGroups = Math.max(1, (int) Math.round(Math.sqrt(agents.size())));
